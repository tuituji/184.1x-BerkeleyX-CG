...
//	glPolygonMode( GL_FRONT_AND_BACK, GL_LINE );
  ...
  if (numused) {
	...
	for (i = 0; i < numused; ++i) {
#if 0
		vec4 tmp =  mv * vec4(lightposn[ 4 * i]);
		lightransf[4 * i] = tmp[0];
		lightransf[4 * i + 1] tmp[1];
		lightransf[4 * i + 2] tmp[2];
		lightransf[4 * i + 3] tmp[3];
#endif
  ...
  transf = mv  *sc * tr ;  
//  glLoadMatrixf(&transf[0][0]);// do not need , will load matrix later 
  ...
  for (int i = 0 ; i < numobjects ; i++) {
	...
	mat4 transf_tmp = transf * obj->transform; 
	glLoadMatrixf(&transf_tmp[0][0]); 
//	glLoadMatrixf(&obj->transform[0][0]);  // notice 
